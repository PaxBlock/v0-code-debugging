        ...
        {activeTab === 'issue' && account && (walletRole === 'owner' || walletRole === 'admin' || walletRole === 'issuer') && (
          <div className="space-y-6">
            <section className="bg-white rounded-xl p-6 border border-pax-900/40 space-y-4">
              ...
              <div className="border-t border-gray-200 pt-4"><div className="flex justify-between items-center"><h3 className="font-semibold">My API requests</h3><button className="text-sm underline" onClick={loadApiRequests}>Refresh</button></div>{apiRequests.map((request) => <div key={request.id} className="mt-3 p-3 border border-gray-200 rounded-lg"><div className="flex justify-between"><span className="font-medium">{request.institution_name}</span><span className="text-xs uppercase">{request.status}</span></div>{request.api_key_prefix && <code className="text-xs block mt-2">{request.api_key_prefix}••••••••</code>}</div>)}</div>
            </section>
            {/* Step 1: Grant Role */}
